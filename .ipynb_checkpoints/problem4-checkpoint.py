...
def count_characters(filename, include_spaces=True):
    ...
    with open(filename, 'r') as f:
        if include_spaces:
            ...
            text = f.read()
            listed_text = text.split()
            lis = [len(i) for i in listed_text]
            counter = sum(lis)
    return counter
...
def word_frequency(filename):
    ...
    with open(filename,'r') as f:
        text = f.read()
    listed_text = text.split()
    listed_text = [i.lower() for i in listed_text]
    clean = [''.join(ch for ch in s if ch not in string.punctuation) for s in listed_text]
    for i in clean:
        if i in frequency:
            frequency[i] += 1
        else:
